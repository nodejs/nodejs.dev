...
[Native Abstractions for Node.js (`nan`)](https://www.npmjs.com/package/nan)
...
[WHATWG URL Standard](https://url.spec.whatwg.org/) was added to Node.js 7.x
...
A number of important changes have been made to the `Buffer` API within Node.js.
...
`Buffer` instance. Prior versions of Node.js would return uninitialized memory,
which could contain potentially sensitive data.

In Node.js 6.0.0, a new set of `Buffer` construction methods
...
cause Node.js to emit `DeprecationWarning` process warnings when `Buffer(num)`
...
Node.js 8.0.0 includes a new `util.promisify()` API that allows standard Node.js
...
`console` module in Node.js allow application output to be directed either to
...
Node.js 8 is the next release line to enter Long Term Support (LTS). This is
scheduled to happen in October 2017. Once Node.js 8 transitions to LTS, it will
receive the code name Carbon.

![Node.js Long Term Support Schedule](/static/images/blog/release/lts.png)

Note that, when referring to Node.js release versions, we have dropped the "v"

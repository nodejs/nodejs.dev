...
export default connectGraphQlArticle(ArticleLayout, {
  editPath: 'content/about/history.md',
  currentSlug: AboutNavigationKeys.history,
...
export const query = graphql`
  query ($locale: String!, $defaultLocale: String!) {
    articleCurrentLanguage: mdx(
      fields: { slug: { eq: "history" }, locale: { eq: $locale } }
    ) {
      body
      tableOfContents
      frontmatter {
    ...
    articleDefaultLanguage: mdx(
      fields: { slug: { eq: "history" }, locale: { eq: $defaultLocale } }
    ) {
      body
      tableOfContents
      frontmatter {

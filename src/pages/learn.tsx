...
export default ({ data, location }: Props) => {
  ...
  for (const { node: page, previous, next } of data.pages.edges) {
    ...
    if (isActive) {
      ...
      previousPage = { frontmatter: { title: previous.frontmatter.title, path: previous.frontmatter.title.toLowerCase().replace(/ /g, '-').replace(/[^a-z0-9|-]/g, '') } };

      nextPage = { frontmatter: { title: next && next.frontmatter.title, path: next && next.frontmatter.title.toLowerCase().replace(/ /g, '-').replace(/[^a-z0-9|-]/g, '') } };
    }

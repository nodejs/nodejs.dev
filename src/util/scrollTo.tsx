...
const easeInOutCubic = (t: number, b: number, c: number, d: number) => ((t /= d / 2) < 1
  ? (c / 2) * t * t * t + b
  : (c / 2) * ((t -= 2) * t * t + 2) + b);
...
export function scrollTo(
  ...
  const start = ((element && element.scrollTop)
      || window.pageYOffset
  ...
  return new Promise((resolve, _reject) => {
    const animateScroll = (): undefined | number => {
